...
class FormFieldUtils {
  ...
  static String? getLabel(FormFieldContext context, {bool getLabel = !UIConstants.labelSeparateText}) {
    if (!getLabel || context.options?.formattingOptions?.label == false) {
      return null;
    }

    String? getScope() {
      final lastScopeElement = context.scope.split('/').last;
      if (lastScopeElement != "items") {
        return lastScopeElement[0].toUpperCase() + lastScopeElement.substring(1);
      } else {
        return null;
      }
    }
  ...
  static String? Function(dynamic) createBaseValidator(
    ...
    return (valueCandidate) {
      ...
      if (context.required || context.options?.formattingOptions?.forceRequired == true) {
